...
\begin{tikzpicture}
    ...
    \node (target) [right=of source] at (7.3,0) {\includegraphics[width=5cm]{bc}};
    ...
    \node[opacity=0.6] (token1) at (1.9,1.6) {\includegraphics[width=1cm]{smartc}};
    \node[opacity=0.6] (token2) at (9,1.6) {\includegraphics[width=1cm]{smartc}};
   
    % \path (wa.east)+(\blockdist,0) node (vote) [sensor] {$\theta_0,\theta_1,...,\theta_M$\\Estimated Parameters};

    % point over event1
    \node (a1) at (3.4,2.6) {Emitted Events};
    % point below block
    \node (a2) at (3.4,-2.4) {};
    ...
    \begin{pgfonlayer}{background}
				...
				\end{pgfonlayer}
    % name of the box
    \node (boxtxt) at (-2,2.7) {\textbf{Source}};

    \path [draw, ->] (event1) -- node [above] {} 
				(asr1.west);
    \path [draw, ->] (block) -- node [above] {} 
				(asr1.west);
    \path [draw, ->] (asr1.east) -- node [above] {$tx_1$} (8.5,1.6) ;

    \path [draw, ->,dotted] (event1) -- node [above] {} 
				(asr2.west);
    \path [draw, ->,dotted] (block) -- node [above] {} 
				(asr2.west);
    \path [draw, ->,dotted] (asr2.east) -- node [above] {$tx_2$} (8.5,1.6) ;

    \path [draw, ->,dotted] (event1) -- node [above] {} 
				(asr3.west);
    \path [draw, ->,dotted] (block) -- node [above] {} 
				(asr3.west);
    \path [draw, ->,dotted] (asr3.east) -- node [above] {$tx_n$} (8.5,1.6) ;
