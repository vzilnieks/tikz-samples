...
\begin{tikzpicture}
  \node (source) {\includegraphics[height=5cm,trim={2.5cm 0 0 0},clip]{bc}};
    ...
    \node (block) at (2.4,-1.4) [event] {zk-Proof Event};
    ...
    \path [draw, ->] (block) -- node [above] {} 
				(asr1.west);
    \path [draw, double, ->] (asr1.east) -- node [above] {$tx_n$, $p_k$} (7.5,1.6) ;

    \path [draw, ->,dotted] (event1) -- node [above] {} 
				(asr2.west);
    \path [draw, ->,dotted] (block) -- node [above] {} 
				(asr2.west);
    \path [draw,double, ->,dotted] (asr2.east) -- node [above] {$tx_n$, $p_k$} (7.5,1.6) ;

    \path [draw, ->,dotted] (event1) -- node [above] {} 
				(asr3.west);
    \path [draw, ->,dotted] (block) -- node [above] {} 
				(asr3.west);
    \path [draw,double, ->,dotted] (asr3.east) -- node [above] {$tx_n$, $p_k$} (7.5,1.6) ;
    ...
    \node[draw, fill=white, rounded corners, text width=3.6cm] at (9,2.3) {
				Transaction Approoved 
		};
