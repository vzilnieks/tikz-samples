...
\begin{tikzpicture}[node distance=2.5cm]
  ...
  \draw (scAB) edge[above] node{\colorbox{black}{\textcolor{white}{\textbf{4}}} Proof of Lock} (rel); 
  \draw (rel) edge[bend left,below] node{Block B Sync} (scAB);
  \draw (rel) edge[above] node{\colorbox{black}{\textcolor{white}{\textbf{5}}} Proof of Lock} (scBA);
  ...
  \draw (scBA) edge[bend right] node{\colorbox{black}{\textcolor{white}{\textbf{7}}} Gossip of Transaction} (c1);
  \draw (c1) edge[bend left, above] node{\colorbox{black}{\textcolor{white}{\textbf{8}}} Committed Block} (bankB);

  \draw (scBA) edge[loop below] node{\colorbox{black}{\textcolor{white}{\textbf{6}}} Check Proof of Lock} (scBA); 
